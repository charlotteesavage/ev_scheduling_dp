...
#adding charging
def cplex_to_df(w, x, d, tt, car_avail, mode, keys, act_id, location, charger_access, soc, charging, charging_duration, charge_time_type1, charge_time_type2, charge_time_type3, charger_level1, charger_level2, charger_level3, minutes = None):
    '''
    Stores a CPLEX solution into a Pandas dataframe.
    '''
    solution_df = pd.DataFrame(columns=['act_id','label', 'start_time', 'end_time', 'duration', 'location', 'charger_access','soc', 'charging','charging_duration', 'charge_time_type1','charge_time_type2','charge_time_type3', 'charger_level1', 'charger_level2','charger_level3', 'mode', 'travel_time'])
    ...
    for idx, name in enumerate(keys):
        if w[name].solution_value == 1:
            ...
            solution_df.loc[idx,'soc']=(soc[name].solution_value)
            solution_df.loc[idx,'charging']=(charging[name].solution_value)
            solution_df.loc[idx,'charging_duration']=(charging_duration[name].solution_value)
            solution_df.loc[idx,'charge_time_type1']=(charge_time_type1[name].solution_value)
            solution_df.loc[idx,'charge_time_type2']=(charge_time_type2[name].solution_value)
            solution_df.loc[idx,'charge_time_type3']=(charge_time_type3[name].solution_value)
            solution_df.loc[idx,'charger_level1']=(charger_level1[name].solution_value)
            solution_df.loc[idx,'charger_level2']=(charger_level2[name].solution_value)
            solution_df.loc[idx,'charger_level3']=(charger_level3[name].solution_value)
            solution_df.loc[idx,'charger_access']=charger_access[name]
...
def create_dicts(df, preferences = None, minutes = False):
    ...
    if 'charger_access' in df.columns:
        charger_access = df.set_index('label')['charger_access'].to_dict()
    else:
        charger_access = None
    ...
    return keys, location, feas_start, feas_end, des_start, des_duration, flex_early, flex_late, flex_short, flex_long, group, mode, act_id, charger_access
#def read_schedule(hh, sched_folder = "MTMC_schedules/", tt_folder = "MTMC_ttmatrices/", mode = "driving", compute = False, verbose = False):
def read_schedule(hh, sched_folder = "CASESTUDY_JOCM/", tt_folder = "CASESTUDY_JOCM/", mode = "driving", compute = False, verbose = False):
    fname_csv = str(hh)
...
def add_new_coordinate(coordinates_dict, new_coord, distances):
    """
    Adds a new coordinate and its distances to and from other coordinates in the nested dictionary.

    :param coordinates_dict: The existing nested dictionary of coordinates and distances.
    :param new_coord: The new coordinate to add, as a tuple (latitude, longitude).
    :param distances: A dictionary mapping each existing coordinate to the distance from the new coordinate.
    """
    # Add the new coordinate with its distances to the existing coordinates
    coordinates_dict[new_coord] = distances
    
    # Update each existing coordinate with the distance to the new coordinate
    for coord, distance in distances.items():
        if coord in coordinates_dict:
            coordinates_dict[coord][new_coord] = distance
        else:
            coordinates_dict[coord] = {new_coord: distance}# add a new dictionary for the new coordinate

    # Ensure the distance from the new coordinate to itself is set to 0
    coordinates_dict[new_coord][new_coord] = 0
