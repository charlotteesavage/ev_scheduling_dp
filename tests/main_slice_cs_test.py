...
def initialize_utility():
    ...
    Remapping strategy:
    Index 0: Not used (groups start at 1)
    Index 1: Home        → use original index 0 params
    Index 2: Work        → use original index 6 params
    Index 3: Business    → use original index 2 params (Errands)
    Index 4: Leisure     → use original index 4 params
    Index 5: Education   → use original index 1 params
    Index 6: Service     → use original index 7 params (ServiceStation)
    Index 7: Delivery    → use original index 2 params (Errands)
    Index 8: Escort/Other→ use original index 3 params (Escort)
    """
    # Original parameters
    orig_asc   = [0, 17.4, 16.1, 6.76, 12, 11.3, 10.6, 0]
    orig_early = [0, -2.56, -1.73, -2.55, -0.031, -2.51, -1.37, 0]
    orig_late  = [0, -1.54, -3.42, -0.578, -1.58, -0.993, -0.79, 0]
    orig_long  = [0, -0.0783, -0.597, -0.0267, -0.209, -0.133, -0.201, 0]
    orig_short = [0, -0.783, -5.63, 0.134, -0.00764, 0.528, -4.78, 0]
    # orig indices:     0(Home), 1(Edu), 2(Errands), 3(Escort), 4(Leisure), 5(Shop), 6(Work), 7(Service)

    # Remap to match actual data groups (after group-1 preprocessing, indices 0-7)
    asc = [
        orig_asc[0],    # Group 0: Home (special - allows multiple visits)
        orig_asc[6],    # Group 1: Work
        orig_asc[2],    # Group 2: Business (use Errands params)
        orig_asc[4],    # Group 3: Leisure/Shop/Visit
        orig_asc[1],    # Group 4: Education
        orig_asc[7],    # Group 5: Service (Depot/Medical)
        orig_asc[2],    # Group 6: Delivery (use Errands params)
        orig_asc[3],    # Group 7: Escort/Other/PT
    ]

    early = [
        orig_early[0],  # Group 0: Home
        orig_early[6],  # Group 1: Work
        orig_early[2],  # Group 2: Business
        orig_early[4],  # Group 3: Leisure
        orig_early[1],  # Group 4: Education
        orig_early[7],  # Group 5: Service
        orig_early[2],  # Group 6: Delivery
        orig_early[3],  # Group 7: Escort/Other
    ]
