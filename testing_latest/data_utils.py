...
def cplex_to_df(
    ...
    for idx, name in enumerate(keys):
        if w[name].solution_value == 1:
            ...
            solution_df.loc[idx, "charging_duration"] = charging_duration[
                name
            ].solution_value
            solution_df.loc[idx, "charge_time_type1"] = charge_time_type1[
                name
            ].solution_value
            solution_df.loc[idx, "charge_time_type2"] = charge_time_type2[
                name
            ].solution_value
            solution_df.loc[idx, "charge_time_type3"] = charge_time_type3[
                name
            ].solution_value
            ...
            if minutes:
                solution_df.loc[idx, "start_time"] = (
                    solution_df.loc[idx, "start_time"] / 60
                )
                solution_df.loc[idx, "duration"] = solution_df.loc[idx, "duration"] / 60
                solution_df.loc[idx, "travel_time"] = (
                    solution_df.loc[idx, "travel_time"] / 60
                )
...
def plot_mode(df, modes=None):
    ...
    plt.yticks(
        np.arange(len(mode_dict)) + 1, labels=list(mode_dict.keys()), fontsize=12
    )
    plt.xlabel(
        "Trip start time [h]",
        fontsize=12,
...
def read_schedule(
    hh,
    sched_folder="CASESTUDY_JOCM/",
    tt_folder="CASESTUDY_JOCM/",
    mode="driving",
    compute=False,
    verbose=False,
):
    ...
    try:
        ...
        sched["location"] = sched.location.apply(
            lambda x: tuple(map(float, x[1:-1].split(",")))
        )  # need to convert locations back to tuple after import
...
def get_param_variables(param_dict):
    ...
    (
        ...
        param_dict["p_st_e_f"],
        param_dict["p_st_l_f"],
        param_dict["p_dur_s_f"],
        param_dict["p_dur_l_f"],
        param_dict["p_st_e_m"],
        param_dict["p_st_l_m"],
        param_dict["p_dur_s_m"],
        param_dict["p_dur_l_m"],
        param_dict["p_st_e_r"],
        param_dict["p_st_l_r"],
        param_dict["p_dur_s_r"],
        param_dict["p_dur_l_r"],
    )
    p_t, error_w, error_z, error_x, error_d = (
        param_dict["p_t"],
        param_dict["error_w"],
        param_dict["error_z"],
        param_dict["error_x"],
    ...
    d_st_h, d_st_w, d_st_edu, d_st_s, d_st_er, d_st_b, d_st_l, d_st_es = (
        param_dict["d_st_h"],
        param_dict["d_st_w"],
        param_dict["d_st_edu"],
        param_dict["d_st_s"],
    ...
    d_dur_h, d_dur_w, d_dur_edu, d_dur_s, d_dur_er, d_dur_b, d_dur_l, d_dur_es = (
        param_dict["d_dur_h"],
        param_dict["d_dur_w"],
        param_dict["d_dur_edu"],
        param_dict["d_dur_s"],
        param_dict["d_dur_er"],
        param_dict["d_dur_b"],
        param_dict["d_dur_l"],
    ...
    for m in mode:
        ...
        try:
                ...
                for l in loc:
                    ...
                    for i in loc:
                        if i == l:
                            ...
                            dic[i] = (
                                gmaps.directions(origin, destination, mode=m)[0][
                                    "legs"
                                ][0]
                                .get("duration", {})
                                .get("value", -1)
            ...
            # except googlemaps.exceptions as gmapserror:
                ...
                print(
                    f"Unexpected error for HH: {h} for mode: {m} - Change this to better error handling if needed"
                )
...
def add_new_coordinate(coordinates_dict, new_coord, distances):
    ...
    for coord, distance in distances.items():
        if coord in coordinates_dict:
            ...
            coordinates_dict[coord] = {
                new_coord: distance
            }  # add a new dictionary for the new coordinate
